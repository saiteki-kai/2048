...
{
    return title_font_size + subtitle_font_size + vertical_gap;
}
...
auto MessageLayout::TitleRect() const -> SDL_FRect
{
    ...
    title_rect.x = rect.x;
    title_rect.y = rect.y + (rect.h - total_height) / 2 - title_font_size + offset_y;
    title_rect.h = title_font_size;
    title_rect.w = rect.w;
...
auto MessageLayout::SubtitleRect() const -> SDL_FRect
{
    ...
    subtitle_rect.y = rect.y + TotalSize() + title_font_size + vertical_gap + offset_y;
    subtitle_rect.h = subtitle_font_size;
...
auto GridLayout::GetTileLayout(const size_t row, const size_t col) const -> TileLayout
{
